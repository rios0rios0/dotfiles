...
- fixed every zsh startup printing ` * Warning: --agents is deprecated, ignoring.` on Android (and any other host running `keychain` `2.9.x`+) by dropping `--agents ssh` from the `keychain --eval` invocation in `dot_zshrc.tmpl`. `keychain` `2.9.x` removed the flag — the default already targets `ssh-agent` and only adopts `gpg-agent` when `--ssh-allow-gpg`/`--ssh-spawn-gpg` is passed, so the flag was redundant and only produced noise on every new shell
