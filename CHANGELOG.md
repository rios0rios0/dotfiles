...
- refreshed `.github/skills/code-review/SKILL.md` and `.github/copilot-instructions.md` to match the current codebase: reworded the secret-hygiene bullet in the `code-review` skill to name the vendor behind each credential prefix and to drop the hyphen from the Slack example, the one shape the shared Gitleaks history scan matches on its own, added the `test-clipboard-shim` target to the `make test` summary, and added the `clipshot` logging prefix
...
- added an `xclip` shim and a `clipshot` stager on Android so Claude Code's Ctrl+V can attach screenshots. Claude Code reads clipboard images by shelling out to `xclip`/`wl-paste`, which can never work under Termux (no X11 or Wayland display, and Android's clipboard cannot pass a bitmap across the app sandbox), so paste always reported "No image found in clipboard". The shim answers the two invocations Claude Code makes and synthesises the clipboard from the newest screenshot in `~/storage`, bounded by a freshness window so a stale image is never attached, and marked consumed after one read so Ctrl+V still falls through to text. Both scripts read the watched folders from a single `dot_local/lib/claude-clipboard.sh`, so the shim and `clipshot -s` cannot disagree about what Ctrl+V would attach, and `make test-clipboard-shim` covers the behaviour against the literal `xclip` command lines
- added the Codex CLI to every platform: `install_codex_cli` installs the `@openai/codex` npm package on Linux/WSL next to Claude Code, `OpenAI.Codex` joins the winget development list on Windows, and Android gets `run_once_before_android-001f-create-codex-wrapper.sh`, a `codex` wrapper for the GitHub release's static `aarch64-unknown-linux-musl` binary. `npm install -g @openai/codex` cannot work on Termux -- the platform package `@openai/codex-linux-arm64` declares `os: linux` while Termux's Node reports `android`, so npm skips it and the launcher throws `Missing optional dependency` -- and the Android installer removes such an install before bootstrapping the wrapper. The wrapper runs the binary through `termux-etc-mount` for DNS (musl reads `/etc/resolv.conf` through direct syscalls), sets `SSL_CERT_FILE` to Termux's CA bundle because rustls probes the usual bundle paths with `stat()`, which Tier 3 does not redirect, defaults `sandbox_mode` to `danger-full-access` because the bubblewrap sandbox needs user namespaces that Android's SELinux policy denies (every other mode panics before running a command), parks `LD_PRELOAD` under the `TERMUX_ETC_LD_PRELOAD` contract, takes `termux-wake-lock`, and handles updates itself since `codex update` refuses manual installs: the first launch downloads the latest release in the foreground, later launches check the `releases/latest` redirect once per 24h in the background (both downloads HTTPS-only, refusing a redirect to plain HTTP like the installer scripts), keep three builds under `~/.local/share/codex/versions/`, and silence Codex's own startup update nag; `CODEX_WRAPPER_NO_AUTO_UPDATE`, `CODEX_WRAPPER_FORCE_VERSION` and `CODEX_WRAPPER_KEEP_SANDBOX` override each part. `make test-script-order` now pins every Android wrapper (`001c` to `001f`) before the installer. The three dependency installers change, so chezmoi re-runs the `run_once_` scripts on the next apply; the guards make that pass mostly a no-op
- added the Sentry CLI (`sentry`, the npm package from `getsentry/cli`, not the classic Rust `sentry-cli`) to the Linux/WSL and Android dependency installers through a shared `install_sentry_cli` in `.chezmoitemplates/lib-install-deps.sh`. The official `cli.sentry.dev/install` script is deliberately not used: it downloads a Bun-compiled `sentry-linux-<arch>` executable linked against glibc (the musl variants stopped shipping after `0.34.0`), which bionic cannot start, so on Termux it dies with `No such file or directory` before `sentry cli setup` runs. The npm package is the same CLI as a plain JavaScript bundle with no platform package and no lifecycle script (`engines.node >= 20`), so it runs under Termux's native Node without any `termux-etc-redirect` wrapper and under NVM on Linux/WSL with one function body, installed with `--ignore-scripts`; `sentry cli upgrade` detects the npm layout and upgrades through npm, and a device with the install verified `sentry --version`, `sentry cli upgrade --check` (network, TLS and `node:sqlite`) and an unauthenticated API call failing at the auth layer rather than at DNS. Both installers change, so chezmoi re-runs the two `run_once_` scripts on the next apply; the guards make that pass mostly a no-op. README, CLAUDE.md, the Copilot instructions and the fresh-machine runbook document the tool, the login flow (`sentry auth login`, a device code that works on the phone). Shell completions and the Claude Code agent skill come from `sentry cli setup --no-modify-path`, run by the new `run_onchange_after_linux-007-setup-sentry-cli.sh` and `run_onchange_after_android-006-setup-sentry-cli.sh` after the managed files are applied, with `SHELL` pinned to zsh; `dot_zshrc.tmpl` puts `~/.local/share/zsh/site-functions` on `fpath` as the quoted absolute path the CLI checks for, because even with `--no-modify-path` the command appends its own `fpath` line to `~/.zshrc` when that path is missing, an edit the next apply would revert. Windows is left out: the Windows installer is winget-only

### Fixed

- fixed `#!/usr/bin/env` scripts exec'd directly from Claude Code tool calls on Termux, which is how the npm-installed Sentry CLI failed with `bad interpreter: /usr/bin/env: no such file or directory` while `zsh -c 'sentry --version'` worked (`copilot`, another npm bin, failed the same way). The `claude` wrapper parks `LD_PRELOAD` in `TERMUX_ETC_LD_PRELOAD` and `~/.zshenv` re-exports it, but an export from inside a running zsh reaches only that shell's children (`make`, `bash -c`): the tool-call shell itself was started without `termux-exec`, so the kernel could not resolve the `/usr/bin/env` shebang of anything the shell exec'd directly. `run_once_before_android-001e-create-claude-wrapper.sh` now also writes `~/.local/bin/claude-zsh`, and the wrapper exports it as `CLAUDE_CODE_SHELL`, which Claude Code honours when the path contains `zsh`/`bash` and is executable (the shell type and rc file are derived from that string): the launcher restores the parked value, drops the `NODE_EXTRA_CA_CERTS` the wrapper sets for Claude's own musl Node (bionic Node never needs it and the Sentry CLI warned about it on every run) and execs the real zsh, so the library is mapped before the shell's first exec. A `CLAUDE_CODE_SHELL` already in the environment wins. The `.zshenv` restore stays as the fallback for a `termux-etc-mount` run by hand, an SSH login and older wrappers, and the `claude-exec-shim` repoint is unchanged: with the library in the shell, `grep`/`find` still reach the musl binary through the shim, and `termux-exec` passes `argv[0]` through untouched (verified: `ugrep 7.8.4` and `bfs 4.1.1` from a shell started by the launcher). The Sentry CLI keeps its npm layout: no per-tool wrapper, no `termux-fix-shebang` (which `sentry cli upgrade` would undo). `001e` changes, so chezmoi re-runs it on the next apply; start a fresh `claude` session afterwards
- fixed `#!/usr/bin/env` scripts exec'd directly from Codex CLI tool calls on Termux -- the other half of the Claude Code fix in this release. `sentry --version` run from a `codex exec` tool call died with `bad interpreter: /usr/bin/env: No such file or directory` (exit 126), and so did any other npm-installed CLI or `#!/usr/bin/env bash` script. `termux-etc-mount` drops `LD_PRELOAD` before the static musl `codex` binary starts and the wrapper parks it in `TERMUX_ETC_LD_PRELOAD`, but nothing handed it back to the shells Codex spawns for tool calls, not even for their children: Codex never consults `$SHELL`. It resolves the shell from the password database (`getpwuid_r`), and Termux's entry is `$PREFIX/bin/login`, which names no shell it knows, so the Linux fallback order applies, `which bash` wins, and a tool call runs `$PREFIX/bin/bash -lc "<cmd>"` -- a shell that never sources `~/.zshenv` (verified on a device: `LD_PRELOAD` unset and no `termux-exec` mapping in the tool shell). A `CLAUDE_CODE_SHELL`-style launcher is no help either, because Codex picks the shell binary itself. `run_once_before_android-001f-create-codex-wrapper.sh` therefore appends `-c 'shell_environment_policy.set.LD_PRELOAD=<parked value>'` to every launch: Codex applies that policy to the environment it spawns tool commands with, so `termux-exec` is mapped before the tool shell's first `execve`, whichever shell Codex picks. The fallback matches `~/.zshenv` (`termux-exec` alone when nothing was parked) and `CODEX_WRAPPER_KEEP_LD_PRELOAD=1` opts out. Verified end to end with `codex exec` on a device: before, `sentry --version` and a `#!/usr/bin/env bash` probe both failed with `bad interpreter` and `grep -c termux-exec /proc/$$/maps` printed `0`; after, `0.44.1`, the probe's marker, and `3`. `001f` changes, so chezmoi re-runs it on the next apply
- fixed `grep` and `find` failing with exit 127 and eleven `Error relocating ... symbol not found` lines inside every Claude Code tool call on Termux. Claude Code appends its own `grep`/`find` functions to the shell snapshot it sources into each tool call, and both run the Claude Code binary itself under another `argv[0]` (`ARGV0=ugrep "$CLAUDE_CODE_EXECPATH" ...`, `ARGV0=bfs ...`) because that binary is multi-call and dispatches on `argv[0]` alone. They reach it through `$CLAUDE_CODE_EXECPATH`, which points straight at the musl build, from a shell where `dot_zshenv.tmpl` has just restored `LD_PRELOAD` for `termux-exec` -- so the musl loader is handed a bionic library it cannot relocate and exits before `main()`. The `claude` wrapper already parks `LD_PRELOAD` for the CLI itself, but it cannot be reused here: it is a `#!` script, and the kernel replaces `argv[0]` with the interpreter path when it runs one, so `ugrep` would be lost and the binary would start as the Claude CLI. The snapshot also defines its functions after `.zshenv` is sourced, so redefining `grep`/`find` there cannot win either. The Android block of `dot_zshenv.tmpl` therefore repoints `$CLAUDE_CODE_EXECPATH` at `~/.local/bin/claude-exec-shim` (parking the real path in `TERMUX_CLAUDE_EXEC_TARGET`), a small C program that new `run_after_android-007-build-claude-exec-shim.sh` compiles with `clang`: it parks `LD_PRELOAD` under the same `TERMUX_ETC_LD_PRELOAD` contract and `execv`s the target with `argv` untouched. Ordinary processes keep `LD_PRELOAD`, so `#!/bin/bash` and `#!/usr/bin/env` shebang rewriting is unaffected; a nested shell will not park the shim as its own target, and a missing shim leaves `$CLAUDE_CODE_EXECPATH` alone. Android only -- the rendered `.zshenv` is byte-identical on Linux and Windows

### Security

- enforced HTTPS-only redirects on the `curl` and `wget` downloads in the validate workflow, the baremetal Linux dependency installer, and the shared font-install library
- locked the `ruff` install in CI to a hash-verified binary wheel instead of resolving an unpinned package with setup scripts allowed
- pinned the third-party GitHub Actions used by the validate workflow to full commit SHAs (`actions/checkout` v7.0.1, `actions/setup-go` v7.0.0)
- replaced the piped `get.chezmoi.io` installer in CI with a pinned release archive that is verified against the checksums published with that release before it is installed

## [0.21.0] - 2026-09-05

### Added

- added `.docs/fresh-machine-setup.md`, a linear runbook for provisioning an empty Windows 11, Kali on WSL or Termux machine with this repository: the 1Password items every platform reads at render time, six ordered phases with every item marked `[automated]` (with the script behind it), `[partial]` or `[manual]`, verification commands per phase, the automation gaps worth turning into issues, and the checklist from issue #30 mapped item by item. Linked it from the README's Installation section, added the missing `winget install twpayne.chezmoi` to the Windows bootstrap, and corrected the "What's Managed" lists: Heroku CLI, ImageMagick and the CycloneDX CLI are not installed by any script, and `pdftk` only exists on Windows and baremetal Linux. Also corrected the `chezmoi state delete-bucket` bucket name in `.github/copilot-instructions.md` from `scriptStates` to `scriptState`, the name `chezmoi state dump` reports
- added `termux-wake-lock` to the `claude` wrapper on Android, taken before the `exec` when `termux-tools` provides it, so Android does not pause the CPU of a backgrounded Termux mid-task. The lock is a Termux-level toggle rather than a per-session one: the wrapper `exec`s, so nothing releases it when Claude exits, and it persists until `termux-wake-unlock` is run by hand. It is idempotent
- added the Fly.io CLI (`install_fly_cli`) and the Oracle Cloud CLI (`install_oci_cli`) to the Linux and Android dependency installers, after the AWS and Azure CLIs. `flyctl` comes from the upstream installer run with `--non-interactive` and without `--setup-path`, so it never edits the rc files chezmoi owns; on Linux `dot_zshenv.tmpl` puts `~/.fly/bin` on PATH, and on Android the static linux/arm64 binary is fronted by a `termux-etc-seccomp` wrapper from `run_after_android-003-wrap-terra-clis.sh`, because unwrapped it cannot resolve DNS (Go's resolver reads the `/etc/resolv.conf` Android does not have and falls back to `[::1]:53`). `oci-cli` is a pip install inside the pyenv Python on Linux; on Android the `oci` SDK's pinned `crc32c==2.8.0` wheel is pre-built with the `crc`, `crypto`, `aes` and `sha2` target features forced through `-Xclang -target-feature`, because the source relies on a GCC pragma Termux's clang ignores and `-march` is overridden by its configuration. Windows stays without both, as it is without the AWS and Azure CLIs. README and the fresh-machine runbook list the two tools and their verification

### Changed

- changed the `common-aliases` `rm`, `cp` and `mv` guards into functions that keep `-i` only when stdin is a terminal, and relaxed zsh's `nomatch` for non-interactive shells in `dot_zshenv.tmpl`. Claude Code's Bash tool snapshots the interactive shell's aliases into its runs, where `-i` reads EOF from a closed stdin and skips the copy with exit 0, and a glob that matches nothing aborted whole command lines that a script would have run
- extracted the install functions whose body is identical on Linux/WSL and Android (`command_exists`, `install_oh_my_zsh`, `install_sdkman`, `install_nvm`) into `.chezmoitemplates/lib-install-deps.sh`, included by both dependency installers with a `template` directive; `run_once_before_linux-002-install-dependencies.sh` became a `.sh.tmpl` for the include. Android now runs the hardened Linux bodies (idempotent download guards, `sdk selfupdate force`, the Node.js LTS version check) while keeping its native `nodejs` early exit, Oh My Zsh is installed `--unattended` on both platforms with the login shell switched explicitly afterwards (`usermod` on Linux, Termux's `chsh -s zsh` on Android, which the interactive installer used to do), and everything provisioned differently per platform (apt repositories versus binary downloads, upstream install scripts versus source builds, pyenv versus native Python) stays in the platform installers. Both rendered installers change, so chezmoi re-runs the two `run_once_` scripts on the next apply; the guards make that pass mostly a no-op. The shared bodies also pin the Oh My Zsh download to HTTPS (`--proto '=https'`) and install corepack with `--ignore-scripts`, closing the two SonarCloud security findings raised on the new file, and the Oh My Zsh, SDKMAN and NVM installers are downloaded to a temp file with `--proto '=https' -fsSL` and an explicit status check by a shared `run_remote_installer` helper instead of being piped from `curl` into `bash` or run through `sh -c` on a command substitution, with the login shell switched to zsh only when the Oh My Zsh install succeeded

### Fixed

- fixed `make` targets and every `#!/usr/bin/env` script failing with exit 127 inside Claude Code tool calls on Termux. The `claude` wrapper has to start the musl binary without `LD_PRELOAD` -- the musl loader cannot relocate Termux's bionic shims and exits before `main()` -- but an unset variable is inherited by every bionic process Claude spawns, so `termux-exec` stopped rewriting shebangs for the shells behind tool calls. The wrapper now parks the value in `TERMUX_ETC_LD_PRELOAD` (the same contract `termux-etc-mount` implements) and the Android block of `dot_zshenv.tmpl` restores it in every shell Claude starts, falling back to `termux-exec` alone when nothing was parked
...
- upgraded `golang.org/x/crypto` from `0.49.0` to `0.55.0` in the `tmplcheck` CI helper (`.github/ci/cmd/tmplcheck`), clearing 13 Dependabot alerts — 7 critical, 2 high, 4 medium — against `golang.org/x/crypto/ssh`, `ssh/agent`, and `ssh/knownhosts`. Dependabot cannot fix these itself: the module lives under `.github/`, which the Dependabot app is not authorised to write to, so every `create_pull_request` call is rejected with a 400 and 11 consecutive update runs failed. `github.com/Masterminds/semver/v3` moved `3.4.0` to `3.5.0` in the same tidy.
...
- fixed the `CONTRIBUTING.md` prerequisites, which told contributors to install chlog with `go install` without listing Go itself
...
- added `make test-shell-credentials` (`.github/ci/scripts/test-shell-credentials.sh`) covering credential and workspace removal against a mock `op`: deletion unsets in the reloading shell and in a terminal that inherited the value, an unreachable vault unsets nothing, pre-manifest caches are still pruned, unrelated variables are never touched, and the cache keeps its `600` mode

### Fixed

- changed `reload-credentials` to bypass the 24h cache TTL with a flag instead of deleting `~/.cache/op-credentials.env` and `~/.cache/op-workspaces.env`, so the reload can still read the names the previous cache exported. This is what lets credentials cached before manifests existed be recognised as the loader's own and removed on the first reload, rather than lingering in the shell indefinitely because nothing could prove where they came from
- fixed `_op_load_references` in `dot_scripts/linux-engineering-op-loader.sh` reporting success when it never reached 1Password. A locked vault, a missing `op` or `jq`, or a failed `op item get` produces exactly the same "not in the response" shape as a deletion, so it now returns non-zero and callers keep the environment they already have — otherwise a brief outage would unset every credential at once
...
- fixed a credential deleted from 1Password surviving in the shell forever, so removing a token from the device note and running `reload-credentials` left the variable exported and every terminal opened afterwards still saw it. Rewriting `~/.cache/op-credentials.env` was all the reload did, and a cache can add a variable but never take one away — the `export` had already put the value in the environment, where it outlives the file both in the shell that loaded it and in every shell forked from that one, since tmux panes, IDE terminals and MCP subshells inherit the environment rather than the cache. Each load in `dot_scripts/linux-engineering-shell-credentials.sh` and `dot_scripts/linux-engineering-workspace-aliases.sh` now records the names 1Password returned in `_OP_CRED_NAMES` / `_OP_WS_NAMES`; because those manifests travel with the environment, a name carried into a shell but absent from a later successful load is provably gone and is `unset` (or unaliased). `dot_zshenv.tmpl` runs the same prune, which is what makes a newly opened terminal drop an inherited credential, and skips it entirely unless a manifest actually changed, so an ordinary shell start pays nothing
