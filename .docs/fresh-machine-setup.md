...
| Item                                                                                                                  | Vault      | Used by                                                                                                                                                                  |
|-----------------------------------------------------------------------------------------------------------------------|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `Chezmoi Key` Secure Note with a `private key` field (the age identity)                                               | `personal` | `run_once_before_linux-005-export-private-key.sh` and `run_once_before_windows-004-export-private-key.ps1` write it to `~/.ssh/chezmoi` so the `encrypted_*.age` files decrypt |
| `Device: <deviceName>` Secure Note                                                                                    | `personal` | `dot_gitconfig.tmpl`, `dot_ssh/*`, the SSH/GPG/PEM key scripts, `dot_config/ggshield`, and the runtime credential loader                                                 |
...
| 2    | `run_once_before_linux-002-install-dependencies.sh`           | `[automated]` | apt packages plus every tool in the table below. 45-120 minutes.                                                                                                                                             |
| 2    | `run_once_before_linux-002-install-dependencies(baremetal).sh` | `[automated]` | Exits immediately on WSL. On baremetal Debian or Ubuntu it adds `barrier`, `bleachbit`, `gimp`, `octave`, `scrcpy`, `unetbootin`, `vlc` and `pdftk`, then Genymotion, Reactotron, R-Linux, Slack and VirtualBox. |
| 3    | `run_once_before_linux-003-configure-dependencies.sh`         | `[automated]` | Symlinks Debian's `batcat` to `~/.local/bin/bat`.                                                                                                                                                            |
| 4    | `run_once_before_linux-004-install-fonts.sh.tmpl`             | `[automated]` | The same three font families into `~/.fonts`, then `fc-cache`.                                                                                                                                              |
| 5    | `run_once_before_linux-005-export-private-key.sh`             | `[automated]` | Signs in with `op signin --account my`, recreates `~/.ssh`, and writes the age identity from 1Password.                                                                                                     |
| 6    | Managed files                                                 | `[automated]` | `.zshrc`, `.zshenv`, `.p10k.zsh`, `.tmux.conf`, `.gitconfig`, `.age_recipients`, `.docker/config.json`, `.config/ggshield`, the `ssh` and `ssh-add` wrappers in `~/.local/bin`, `.scripts/*`, `~/.claude.json` (MCP servers), and the decrypted `.kube`, `.histdb`, `.john`, `.sqlmap` and `.npmrc`. `.ssh/config` is Windows and Android only. |
| 7    | `run_once_after_linux-001-extract-compressed-folders.sh`      | `[automated]` | Unpacks the decrypted `.histdb`, `.john`, `.kube/config-files` and `.sqlmap` archives.                                                                                                                      |
| 8    | `run_once_after_linux-002-clone-pentesting-tools.sh`          | `[automated]` | Clones VHostScan, dirsearch, StegCracker and stegbrute into `~/Development/Tools`.                                                                                                                           |
| 9    | `run_after_linux-002-import-gpg-keys.sh.tmpl`                 | `[automated]` | Imports the GPG keys the device note references.                                                                                                                                                             |
| 10   | `run_after_linux-004-install-ggshield-hook.sh`                | `[automated]` | Generates the global GitGuardian pre-commit hook; `core.hooksPath` in `.gitconfig` points at it.                                                                                                             |
| 11   | `run_after_linux-005-install-jetbrains-themes.sh`             | `[partial]`   | Same as Windows: needs the IDE config directories to exist first.                                                                                                                                            |
| 12   | `run_onchange_after_linux-006-remove-dependencies.sh.tmpl`    | `[automated]` | Tombstones; silent on a fresh machine.                                                                                                                                                                       |
