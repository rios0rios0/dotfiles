...
| 14   | `run_onchange_after_linux-007-setup-sentry-cli.sh`           | `[automated]` | `sentry cli setup --no-modify-path`: the zsh completion into `~/.local/share/zsh/site-functions`, and the Claude Code skill once `~/.claude` exists (the next `sentry cli upgrade` adds it otherwise). |
...
| Sentry CLI                                       | `install_sentry_cli`                 | `[automated]` | npm package (`sentry`, from getsentry/cli), shared with Android through `lib-install-deps.sh`; completions and the agent skill come from step 14.                                  |
...
ls ~/.local/share/zsh/site-functions/_sentry
...
| Flutter `3.47.5` (community build)                                | `install_flutter`                                                                 | `[partial]`   | Flutter publishes no Linux arm64 host SDK, so this is [GeneralKaos666/flutter-for-termux](https://github.com/GeneralKaos666/flutter-for-termux) — mumumusuc's bionic cross-build as a Termux `.deb` — pinned to a version and a SHA-256 computed from the inspected file (the project's README hash did not match its asset), `apt`-installed under `$PREFIX/opt/flutter` with `x11-repo` for its gtk3 dependency, and fronted by two bash wrappers in `~/.local/bin`. **Not yet run on a device**: `flutter --version` is the hard check, `precache --web` is reported and not fatal. Termux's own `dart` package (above) is the baseline for `dart format` either way. |
...
| Sentry CLI                                                        | `install_sentry_cli`                                                              | `[automated]` | npm package under Termux's native Node, no wrapper: the bundle is plain JavaScript. The official install script cannot be used here, its Bun-compiled glibc binary does not start on bionic. Completions and the agent skill come from step 10. |
...
| Fly.io CLI                                                         | `install_fly_cli`                                                                  | `[automated]`  | Static linux/arm64 Go binary from the upstream installer into `~/.fly/bin`; wrapped by `run_after_android-003` because it cannot resolve DNS unwrapped.                                                                                                         |
